...
class NeuralNetMLP:
    ...
    def forward(self, x):
        ...
        z_h1 = np.dot(a_h1, self.weight_h2.T) + self.bias_h2
        a_h2 = sigmoid(z_h1)
    ...
    def backward(self, x, a_h, a_h2, a_out, y):  
        ...
        # output dim: [n_examples, n_hidden]
        d_loss__a_h = np.dot(d_loss__a_h2, d_z_out__a_h)
...
ignore1,ignore2, probas = model.forward(X_valid)
...
def compute_mse_and_acc(nnet, X, y, num_labels=10, minibatch_size=100):
    ...
    for i, (features, targets) in enumerate(minibatch_gen):
        ...
        ignore1,ignore2, probas = nnet.forward(features)
        predicted_labels = np.argmax(probas, axis=1)
...
def train(model, X_train, y_train, X_valid, y_valid, num_epochs,
    ...
    for e in range(num_epochs):
        ...
        for X_train_mini, y_train_mini in minibatch_gen:
            ...
            #### Compute gradients ####
            d_loss__d_w_out, d_loss__d_b_out, d_loss__d_w_h, d_loss__d_b_h, d_loss__d_w_h2, d_loss__d_b_h2 = \
...
ignore1,ignore2, probas = model.forward(X_test_subset)
...
# get_ipython().system(' python ../.convert_notebook_to_script.py --input ch11.ipynb --output ch11.py')
